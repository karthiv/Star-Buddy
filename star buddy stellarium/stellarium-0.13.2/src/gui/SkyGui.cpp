...
void InfoPanel::setTextFromObjects(const QList<StelObjectP>& selected, int click)
{
	if (selected.isEmpty())
	{
		if (!document()->isEmpty())
	...
	else
	{
		...
		setHtml(s);
		 QString str1=s.mid(s.indexOf("Alt:")+4,s.indexOf("(apparent)<br>Eclip")-7);
    QStringList str2=str1.split("/");
    //QTextStream(stdout) << str2[0].split("°").first().trimmed() << endl;
    //QTextStream(stdout) << str2[1].split("°").first() .trimmed()<< endl;
	qApp->setProperty("global_var_x",str2[0].split("°").first().trimmed());
		qApp->setProperty("global_var_y",str2[1].split("°").first().trimmed());
